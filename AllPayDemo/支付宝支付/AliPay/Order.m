...
- (NSString *)description {
    ...
    if (self.amount) {
        ...
        if ([self.isInternet isEqualToString:@"1"])
        {
            [discription appendFormat:@"&rmb_fee=\"%@\"", self.amount];
        }
        else
        {
    ...
    if (self.product_code) {
        [discription appendFormat:@"&product_code=\"%@\"",self.product_code];//30m
    }
    if ([self.isInternet isEqualToString:@"1"]) {
        
        [discription appendFormat:@"&currency=\"%@\"",@"HKD"];
        [discription appendFormat:@"&forex_biz=\"%@\"",@"FP"];
        [discription appendFormat:@"&split_fund_info=\"%@\"",self.splitFoundInfo];
    }
